...
// export default class HelloWorld extends BaseVueClass {
    export default class ${NAME}
        extends AbstractBaseVueClass_NotDirectExtendedByMixins.Mixins(DialogMixin) {    // 混入在此处，进行添加。
        ...
        public watch_dialogType(newVal: any, oldVal: any) {
            if (newVal === 1) {
                this.dialogTitle = this.${DS}t('dialog.Create').toString()        // 新建对话框的标题
            } else {
                this.dialogTitle = this.${DS}t('table.Edit').toString()           // 编辑对话框的标题
            }
        }
